...
export function UpsellModal({ open, onClose, plans }: UpsellModalProps) {
  ...
  const paidPlans = plans.filter(p => p.level > 0)

  async function handleUpgrade(plan: SubscriptionPlan) {
    try {
      await buySubscription({ planId: plan.id, periodType: 'MONTHLY' })
      toast({ title: `Subscribed to ${plan.name}`, color: 'success' })
      onClose()
    } catch (err) {
      ...
      <Dialog.Portal>
        ...
        >
          ...
          <Dialog.Description className="mb-6 text-sm text-neutral-500">
            Your FREE plan allows 1 postal operator. Upgrade to connect more.
          </Dialog.Description>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-neutral-500">
                      {plan.maxOperators > 100 ? 'Unlimited operators' : `Up to ${plan.maxOperators} operators`}
                    </p>
                  </div>
                </div>
                <Button
