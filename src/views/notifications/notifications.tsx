...
const TYPE_META: Record<NotificationType, { icon: LucideIcon; iconBg: string; iconColor: string }> = {
  SHIPMENT_STATUS: { icon: Package, iconBg: "bg-teal-100", iconColor: "text-teal-700" },
...
function getTypeLabel(type: NotificationType) {
  switch (type) {
    case "SUBSCRIPTION":
      return translate("notificationsPage.subscription");
    case "POSTAL_CONNECTION":
      return translate("notificationsPage.postalConnection");
    case "ACCOUNT":
      return translate("notificationsPage.account");
    case "SYSTEM":
      return translate("notificationsPage.system");
    case "SHIPMENT_STATUS":
      return translate("notificationsPage.shipmentStatus");
  }
}
...
function formatTime(dateStr: string): string {
  ...
  if (diff < 60) return translate("common.justNow");
  if (diff < 3600) return translate("common.minutesAgo", { count: Math.floor(diff / 60) });
  if (diff < 86400) return translate("common.hoursAgo", { count: Math.floor(diff / 3600) });
  if (diff < 604800) return translate("common.daysAgo", { count: Math.floor(diff / 86400) });
...
export default function NotificationsPage() {
  ...
  const typeOptions: { value: NotificationType | ""; label: string }[] = [
    ...
    { value: "SUBSCRIPTION", label: getTypeLabel("SUBSCRIPTION") },
    { value: "POSTAL_CONNECTION", label: getTypeLabel("POSTAL_CONNECTION") },
    { value: "ACCOUNT", label: getTypeLabel("ACCOUNT") },
    { value: "SYSTEM", label: getTypeLabel("SYSTEM") },
    { value: "SHIPMENT_STATUS", label: getTypeLabel("SHIPMENT_STATUS") },
  ];
    ...
    <main className="py-10">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-2.5">
              <h1 className="text-2xl font-semibold text-neutral-900">{t("notificationsPage.title")}</h1>
              {unreadCount > 0 && (
                <span className="inline-flex h-5 min-w-5 items-center justify-center rounded-full bg-teal-700 px-1.5 text-xs font-semibold leading-none text-white">
                  {unreadCount}
                </span>
              )}
            </div>
            <p className="mt-1 text-sm text-neutral-400">{t("notificationsPage.subtitle")}</p>
          </div>

          <div className="flex flex-wrap items-center gap-2">
            {confirmDeleteAll ? (
              <>
                <span className="text-sm text-neutral-500">{t("notificationsPage.deleteAllQuestion")}</span>
                <button
                  onClick={() => {
                    deleteAll.mutate();
                    setConfirmDeleteAll(false);
            ...
            <select
              ...
              className="h-8 rounded-lg border border-neutral-300 bg-white pl-3 pr-7 text-sm font-medium text-neutral-600 transition-colors hover:border-neutral-400 focus:outline-none focus:ring-2 focus:ring-teal-700/20"
            >
              {typeOptions.map((o) => (
            ...
            <div className="flex rounded-lg border border-neutral-200 bg-neutral-100 p-0.5">
              {(["all", "unread"] as Tab[]).map((value) => (
                ...
                >
                  {value === "all" ? t("notificationsPage.allTab") : t("notificationsPage.unreadTab")}
                </button>
              ))}
            </div>
          </div>
      ...
      {!isLoading && error && (
        <div className="rounded-xl border border-error-100 bg-error-100 px-6 py-10 text-center">
          <p className="text-sm text-error-600">{t("notificationsPage.failedToLoad")}</p>
        </div>
      )}
              ...
              <div className="flex flex-col items-center justify-center py-20 text-center">
                ...
                <p className="text-sm font-medium text-neutral-600">
                  {tab === "unread"
                    ...
                    : typeParam
                      ? t("notificationsPage.noTypeNotifications", { type: getTypeLabel(typeParam as NotificationType) })
                      : t("notificationsPage.noNotificationsYet")}
                </p>
                <p className="mt-1 text-xs text-neutral-400">
                  {tab === "unread"
                    ? t("notificationsPage.noUnread")
                    : t("notificationsPage.notificationsWillAppear")}
                </p>
              </div>
            )}
                ...
                <div key={n.id} className="animate-fade-up" style={{ animationDelay: `${index * 35}ms` }}>
                  <div
                    className={`group relative flex items-start gap-4 rounded-xl border bg-white p-5 transition-all duration-150 hover:shadow-md ${
                      n.isRead ? "border-neutral-200" : "border-neutral-200 border-l-2 border-l-teal-700"
                    }`}
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2">
                        ...
                        {!n.isRead && <span className="h-1.5 w-1.5 shrink-0 rounded-full bg-teal-700" />}
                      </div>
                    ...
                    <div className="flex shrink-0 gap-1 opacity-0 transition-opacity duration-150 group-hover:opacity-100">
                      {!n.isRead && (
                        <button
                          ...
                          aria-label={t("notificationsPage.markAsRead")}
                          title={t("notificationsPage.markAsRead")}
                          className="flex h-8 w-8 items-center justify-center rounded-lg text-neutral-400 transition-colors hover:bg-teal-100 hover:text-teal-700 disabled:opacity-50"
                        >
                      ...
                      <button
                        ...
                        title={t("notificationsPage.deleteNotification")}
          ...
          {totalPages > 1 && (
            <div className="mt-6 flex items-center justify-center gap-3">
              ...
              <span className="text-sm text-neutral-500">
                {t("notificationsPage.pageOf", { page, total: totalPages })}
              </span>
              <button
                onClick={() => setPage(page + 1)}
